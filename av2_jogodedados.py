...
#Função de exibição do Menu para as escolhas de alternativas do programa.
def exibeMenu():
...
#Função de Rolagem de Dados, principal mecânica de um jogo de aventura RPG
def rolar_dado():
    return random.randint(1, 20)

#Função de Inicialização de jogo
def iniciarJogo():
    nome = input("Digite seu nome: ")
    opcao = input(f"{nome}, seus passos ecoam nos seus ouvidos. Você chegou na caverna. O frio da floresta sumiu ao descer, começou a esquentar.\nDigite 1 para Rolar Percepção: ")
    ...
    if teste_percepcao == 20:
        print("Acerto crítico! Você vê o dragão no fundo da caverna, estava quase perfeitamente camuflado... mas está dormindo, você tem a vantagem, Digite 1 para Rolar Ataque!\n")
        ataque = input("Digite 1 para Rolar Ataque: ")
        if ataque == "1":
            resultado = rolar_dado()
            print(f"Você rolou: {resultado}!")
            if resultado >= 10:
        ...
        if esquiva == "1":
            ...
            if teste_esquiva >= 10:
                ataque = input("Com uma reação rápida, você desviou do ataque do dragão! Agora é hora de se vingar! Digite 1 para Rolar Ataque e derrotar o Dragão!: ")
                if ataque == "1":
                    teste_ataque = rolar_dado()
                    print(f"Você rolou: {teste_ataque}!")
                    if teste_ataque >= 10:
                        print(f"Um ataque preciso! Você empala o dragão, que se debate de dor e sente medo, inicando um voo rápido para fora da caverna, fugindo e deixando CodeVille em paz! Parabéns, {nome}!")
...
#Função de mostrar a história contextualizada do jogo
def mostrarHistoria():
...
#Loop de repetição do jogo, apenas finalizando se a opção 3 for escolhida no terminal.
while True:
    opcaomenu = exibeMenu()
